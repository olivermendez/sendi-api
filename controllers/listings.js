...
exports.updateListing = asyncHandler(async (req, res, next) => {
	...
	if(listing.user.toString() !== req.user.id){
		return next(
			new errorResponse(`${req.params.name} You are not authorized to update this listing`, 401)
		);
	}
...
exports.deleteListing = asyncHandler(async (req, res, next) => {
	...
	if(listing.user.toString() !== req.user.id){
		return next(
			new errorResponse(`You are not the Owner, so, you're not authorized to update this listing`, 401)
		);
	}
